...
DEVICEID_XOR_KEY = '3go8&$8*3*3h0k(2)2'
...
def get_anonimous_username(device_id: str) -> str:
    xored_chars = []
    key_length = len(DEVICEID_XOR_KEY)
    for i, ch in enumerate(device_id):
        xored_char = chr(ord(ch) ^ ord(DEVICEID_XOR_KEY[i % key_length]))
        xored_chars.append(xored_char)
    xored_string = ''.join(xored_chars)
    md5_digest = hashlib.md5(xored_string.encode('utf-8')).digest()
    combined_str = f"{device_id} {b64encode(md5_digest).decode('utf-8')}"
    return b64encode(combined_str.encode('utf-8')).decode('utf-8')
