...
static VkPipeline ImGui_ImplVulkan_CreatePipeline(VkDevice device, const VkAllocationCallbacks* allocator, VkPipelineCache pipelineCache, const ImGui_ImplVulkan_PipelineInfo* info)
{
    ...
    // Layrics local patch: layrics presents with VK_COMPOSITE_ALPHA_PRE_MULTIPLIED,
    // so the fragment output must be premultiplied. The custom fragment shader
    // (ImGui_ImplVulkan_InitInfo::CustomShaderFragCreateInfo) premultiplies the
    // vertex color; blending with ONE is the premultiplied over operator.
